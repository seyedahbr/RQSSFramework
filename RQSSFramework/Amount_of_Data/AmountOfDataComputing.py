...
class AmountOfDataComputer:
    ...
    def ref_literal_per_ref_triple_ratio(self):
        ...
        return num_ref_literal / num_ref_triple

    def __repr__(self):
        return """ref node per statement ratio,ref triple per statement ratio,ref triple per ref node ratio,ref literal per ref triple ratio
{0},{1},{2},{3}""".format(
            self.ref_node_per_statement_ratio,
            self.ref_triple_per_statement_ratio,
            self.ref_triple_per_ref_node_ratio,
            self.ref_literal_per_ref_triple_ratio)
